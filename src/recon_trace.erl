...
-define(TRACE_TABLE, trace_table).
...
calls({Mod, Fun, Args}, Max, Opts) ->
    calls({Mod,Fun,Args, low}, Max, Opts);
...
stop_low_priority_trace() ->
    ...
    {PidSpecs, TraceOpts, MatchOpts} = validate_opts(Opts),
    Matches = [begin
                {Arity, _Spec} = validate_tspec(Mod, Fun, Args),
                erlang:trace_pattern({Mod, Fun, Arity}, false, MatchOpts)
               end || {Mod, Fun, Args, low} <- TSpecs],
    [erlang:trace(PidSpec, true, [call, {tracer, Pid} | TraceOpts])
     || PidSpec <- PidSpecs],
    lists:sum(Matches).
...
create_table() ->
    case ets:info(?TRACE_TABLE, name) of
        undefined ->
			need_created;
		_ ->
			ets:delete(?TRACE_TABLE)
	end,
	ets:new(?TRACE_TABLE, [named_table, public]).
...
insert_into_table(tspecs, TSpecs) ->
    ets:insert(?TRACE_TABLE, {tspecs, TSpecs});
insert_into_table(pid, Pid) ->
    ets:insert(?TRACE_TABLE, {pid, Pid});
insert_into_table(opts, Opts) ->
    ets:insert(?TRACE_TABLE, {opts, Opts}).

get_tspecs() ->
    Prolists = ets:tab2list(?TRACE_TABLE),
    proplists:get_value(tspecs, Prolists).

get_pid() ->
    Prolists = ets:tab2list(?TRACE_TABLE),
    proplists:get_value(pid, Prolists).

get_opts() ->
    Prolists = ets:tab2list(?TRACE_TABLE),
    proplists:get_value(opts, Prolists).